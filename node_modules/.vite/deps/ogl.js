...
var Program = class {
  constructor(gl, {
    ...
    gl.shaderSource(vertexShader, vertex9);
...
${addLineNumbers(vertex9)}`);
    ...
    gl.shaderSource(fragmentShader, fragment8);
...
function decodeImage(src, flipY) {
  return new Promise((resolve) => {
    if (isCreateImageBitmap()) {
      fetch(src, { mode: "cors" }).then((r) => r.blob()).then((b) => createImageBitmap(b, { imageOrientation: flipY ? "flipY" : "none", premultiplyAlpha: "none" })).then(resolve);
    } else {
      const img = new Image();
      img.crossOrigin = "";
      img.src = src;
...
var GLTFLoader = class {
  ...
  static async parse(gl, desc, dir) {
    ...
    if (((_a = desc.extensionsRequired) == null ? void 0 : _a.includes("KHR_texture_basisu")) && !this.basisManager)
